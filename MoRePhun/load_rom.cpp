...
bool MophunVM::loadRom(const std::string& romPath)
{
	...
	return loadRom(romData.data(), romData.size());
}

bool MophunVM::loadRom(const uint8_t* romData, size_t romSize)
{
	if (romData == nullptr || romSize < sizeof(VMGPHeader) || romSize > memory.ram.size())
	{
		std::cerr << "ROM size is outside the supported range" << std::endl;
		return false;
	}
